...
export const createPost = async (req, res) => {
    try {
        ...
        if (img) {
            const uploadedResponse = await cloudinary.uplaoder.upload(img);
...
export const commentOnPost = async (req, res) => {
    try {
        ...
        if (!text) {
            return res.status(400).josn({ error: "text field is required" });
        }
        const post = await Post.findById(postId);
        if (!post) {
            return res.statu(400).josn({ error: "Post not found" });
        }
